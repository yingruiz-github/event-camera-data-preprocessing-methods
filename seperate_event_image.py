...
def main():
	...
	for original_folder_name in os.listdir(args.dir_to_read):
		...
		os.makedirs(current_abs_neg)
		os.makedirs(current_abs_pos)
		...
		for event_image_name in os.listdir(event_imgs_abs):
			event_image_name_abs = os.path.join(event_imgs_abs, event_image_name)
			event_image = cv.imread(event_image_name_abs)
			check_pixel_correctness(event_image, PIXEL_SET, event_image_name) # all event images should only has 3 pre-defined pixel values, if not, print it to cmd
			pos_img, neg_img = seperate_pos_neg(event_image, neg = (30, 30, 220), pos = (200, 30, 30))
			# save imgs
			#print("***************************************",np.unique(pos_img, return_counts=True))
			pos_img = Image.fromarray(pos_img)
			neg_img = Image.fromarray(neg_img)
			#pos_img = pos_img.convert('RGB')
			#neg_img = neg_img.convert('RGB')
			neg_img.save(os.path.join(current_abs_neg, "{}_neg.png".format(event_image_name)))
			pos_img.save(os.path.join(current_abs_pos, "{}_pos.png".format(event_image_name)))
			#exit()
